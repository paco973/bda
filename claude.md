...
│   ├── updates.py              # Vérification facultative d'une nouvelle version (accès réseau)
│   ├── assets/
...
│   │   ├── scrape.py           # Récupération du corpus branham.fr (partagé script CLI / bouton intégré)
...
- **`logos/data/scrape.py`** : récupération du corpus de prédications depuis branham.fr — index, parseurs (`parse_index`, `parse_paragraphs`) et `download_corpus(letters, limit, delay, on_progress, should_stop)`. Logique **partagée** entre le script `scripts/scrape_predications.py` (qui n'en est que l'habillage CLI) et le bouton « Télécharger les prédications… » de l'application. Avec `updates.py`, l'un des **deux seuls** modules à accéder au réseau, uniquement sur action explicite de l'opérateur. Pas de Qt : réseau simulé dans les tests. Le corpus téléchargé en appli est déposé via `predications.save_user_corpus()` dans `~/.bda/assets/` (prioritaire sur l'asset livré, il survit aux mises à jour) puis importé aussitôt.
...
- **`logos/updates.py`** : **seul** module qui accède au réseau. Interroge un manifeste JSON (`MANIFEST_URL`, **vide par défaut** → fonctionnalité dormante, aucun appel réseau) et renvoie un `CheckResult` de statut `DISABLED`/`ERROR`/`UP_TO_DATE`/`AVAILABLE`. **Ne lève jamais** et ne télécharge ni n'installe rien : il signale, l'opérateur décide. Le contenu distant étant non fiable, le schéma **HTTPS est imposé** (manifeste *et* lien de téléchargement), la réponse est bornée en taille et les champs texte tronqués. Le contexte TLS retombe sur les certificats de `certifi` (dépendance d'exécution) quand le système n'en fournit aucun — cas d'une appli gelée PyInstaller, où l'échec silencieux donnerait sinon « serveur injoignable » sur tous les postes. Pas de Qt, donc testable tel quel (`tests/test_updates.py` simule `urlopen`).
...
- L'application doit fonctionner **100% hors ligne** — ne pas introduire de dépendance réseau sans en discuter (voir exigences non fonctionnelles du cahier des charges). **Deux exceptions admises**, sous conditions strictes : la vérification de version de `logos/updates.py` — facultative (désactivable par « Aide → Vérifier au démarrage », dormante tant que `MANIFEST_URL` est vide), en tâche de fond, silencieuse en cas d'échec — et le téléchargement du corpus de prédications (`logos/data/scrape.py`) — jamais automatique, uniquement sur action explicite de l'opérateur après une confirmation rappelant copyright et durée, annulable, en tâche de fond. Aucune des deux n'a de conséquence sur le fonctionnement si le poste n'a pas de connexion ; toute autre fonctionnalité doit rester utilisable sans réseau.
